...
def updateDisplay(pupitre_voltage: int, etau_voltage: int, switch_state: str, motor_position: int, step_display: int, analog_button: int) -> None:
        switch_state_txt = 'Fermé' if switch_state == 'O' else 'Ouvert'
        gauge_orientation = str(180 * analog_button / 100)
        ...
        sendNextion(f'z0.val=' + gauge_orientation)
        sendNextion(f'n5.val=' + str(step_display))
        sendNextion(f't5.txt=' + switch_state_txt) # todo
